...
`blume`, `sonne`, eigene Bild-Motive aus `lib/motifs.js` wie `zwinker`; beim Bildmagneten `fuchs`, `eule`, `panda`, `hase`, `lola`, `lolacomic`, `brunocomic`, `melody`; Muster und Motiv schließen sich aus, bei beiden gilt das Motiv), `paper` (`glatt`, `liniert`, `kariert`),
...
Überschrift, unterstrichen und ohne Listenmarkierung; auf liniertem und kariertem Papier belegt sie eine Linienzeile und
ragt nach oben darüber hinaus, damit der Rest auf den Linien bleibt), `tsize` (`1`, `2`, `3`: Größe der
