...
const cutDir = process.env.SP ? `${process.env.SP}/motifs` : '/tmp';   // Kontrollkopie der freigestellten Fassung
fs.mkdirSync(cutDir, { recursive: true });
fs.writeFileSync(`${cutDir}/${key}-cut.webp`, Buffer.from(out.url.split(',')[1], 'base64'));
