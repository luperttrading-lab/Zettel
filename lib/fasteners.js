...
(function (root, factory) {
  ...
  const FASTENERS = {
    ...
    photo:     { label: 'Bildmagnet',    short: 'Bild' },   // eigenes Bild ist der Magnet (lib/motifs.js, kind: 'photo')
  };
  const FASTENER_ALIAS = { klebestreifen: 'tape', streifen: 'tape', zwei: 'tape2', ecken: 'tape2', reisszwecke: 'thumbtack', reißzwecke: 'thumbtack', zwecke: 'thumbtack', pinnadel: 'pushpin', nadel: 'pin', nadeln: 'pin2', klammer: 'clip', bueroklammer: 'clip', büroklammer: 'clip', magnet: 'magnet', bildmagnet: 'photo', bild: 'photo' };
  // Wie weit die Befestigung ins Papier ragt (Vielfaches von p): darunter beginnt erst der Text
  const FASTENER_INSET = { tape: 0, tape2: 0, thumbtack: 0.6, pushpin: 0.9, pin: 0.7, pin2: 0.7, clip: 0, magnet: 1.1, photo: 1.1 };
  // Befestigungen ohne Farbwahl (das Bild bringt alles mit)
  const COLOR_SUPPORT = { photo: false };
  const hasColor = kind => COLOR_SUPPORT[kind] !== false;
  ...
  const FCOLOR_DEFAULT = { tape: 'white', tape2: 'white', thumbtack: 'red', pushpin: 'red', pin: 'red', pin2: 'red', clip: 'white', magnet: 'red', photo: 'white' };
  ...
  // Eigene Bild-Motive (lib/motifs.js): { key: { label, src: dataUri } } – füllen die ganze Magnetfläche
  const MOTIFS = {};   // runde Motive (Magnet)
  const PHOTOS = {};   // Bildmagnete: das Bild ist der Magnet
  function setMotifs(reg) {
    for (const [k, m] of Object.entries(reg || {})) {
      // Anzeigename als Alias, dazu der Schlüssel auf sich selbst – Letzterer nur, wenn der
      // Name noch frei ist, damit ein Motiv namens "herz" nicht das Herz-Symbol verdeckt.
      DECOR_LABEL[k] = m.label; DESIGN_ALIAS[m.label.toLowerCase()] = k;
      const self = k.toLowerCase(); if (!DESIGN_ALIAS[self] && !DESIGNS[self] && !PATTERNS[self]) DESIGN_ALIAS[self] = k;
      if (m.kind === 'photo') { PHOTOS[k] = m; continue; }
      MOTIFS[k] = m; DESIGNS[k] = m.label;
      if (!DESIGN_SUPPORT.magnet.includes(k)) DESIGN_SUPPORT.magnet.push(k);
    }
  }
  ...
  function decorsFor(kind) { return kind === 'photo' ? Object.keys(PHOTOS) : ['none', ...(PATTERN_SUPPORT[kind] || []), ...(DESIGN_SUPPORT[kind] || [])]; }
  ...
  function normalizeLook(kind, look) {
    ...
    if (!decorsFor(kind).includes(decor)) decor = kind === 'photo' ? (Object.keys(PHOTOS)[0] || 'none') : 'none';
    const pattern = PATTERNS[decor] && decor !== 'none' ? decor : 'none';
    const design = (DESIGNS[decor] || PHOTOS[decor]) && decor !== 'none' ? decor : 'none';
  ...
  function design(key, ink, base) {
    if (MOTIFS[key]) { // Bild: rund auf die Scheibe geschnitten
      const id = uid();
      return `<clipPath id="${id}"><circle cx="0" cy="0" r="0.62"/></clipPath><image href="${MOTIFS[key].src}" x="-0.62" y="-0.62" width="1.24" height="1.24" preserveAspectRatio="xMidYMid slice" clip-path="url(#${id})"/>`;
    }
    switch (key) {
  ...
  const SHAPE = {
    ...
    // Büroklammer: Muster als Strichelung – Grundfarbe über einer Gegenfarbe (weiß bzw. dunkel), damit es sich abhebt
    clip: (cx, cy, s, deg, c, pat) => {
      const dash = pat === 'stripes' ? ' stroke-dasharray="0.14 0.14"' : pat === 'dots' ? ' stroke-dasharray="0.001 0.2"' : '';
      const d = 'M-0.3,1.5 V-0.9 a0.3,0.3 0 0 1 0.6,0 V1.6 a0.45,0.45 0 0 1 -0.9,0 V-0.4';
      // mit Muster: dunkle Außenkante (hebt sich vom Papier ab) → Gegenfarbe → Grundfarbe gestrichelt
      const layers = dash
        ? `<path d="${d}" stroke="${c.darker}" stroke-width="0.16"/><path d="${d}" stroke="${c.ink}" stroke-width="0.11"/><path d="${d}" stroke="${c.main}" stroke-width="0.075"${dash}/>`
        : `<path d="${d}" stroke="${c.darker}" stroke-width="0.13"/><path d="${d}" stroke="${c.main}" stroke-width="0.08"/>`;
      return `<g transform="translate(${cx} ${cy}) rotate(${deg}) scale(${s})" fill="none" stroke-linecap="round" stroke-linejoin="round">${layers}</g>`;
    },
  ...
  // Bildmagnet: quadratisch, gleiche Größe wie der runde Magnet; weicher Schatten aufs Papier, sonst nur das Bild
  function photoShape(W, H, p, key) {
    const ph = PHOTOS[key]; if (!ph) return '';
    const g = magnetGeometry(W, H, p), size = 2 * g.r, x = g.cx - g.r, y = g.cy - g.r;
    return `<rect x="${x + size * 0.03}" y="${y + size * 0.07}" width="${size * 0.94}" height="${size * 0.94}" rx="${size * 0.14}" fill="#000" fill-opacity=".28"/>`
      + `<image href="${ph.src}" x="${x}" y="${y}" width="${size}" height="${size}" preserveAspectRatio="xMidYMid meet"/>`;
  }
  // Zweite Nadel bei „Zwei Nadeln“: Gegenfarbe zur gewählten
  const partner = key => FCOLORS[key === 'red' ? 'blue' : 'red'];

  // ---------- Mehrere Befestigungen: Regeln, Standardsitz, Grenzen ----------
  // EINE Quelle für App und Server. Wer hier etwas ändert, ändert es in beiden.
  //   frei  = überall auf dem Zettel (Nadeln, Magnete – sie halten auch mitten im Text)
  //   quer  = nur Ober- oder Unterkante, immer waagerecht (Klebestreifen)
  //   oben  = nur an der Oberkante, seitlich verschiebbar (Büroklammer)
  //   fest  = sitzt, wo sie sitzt (Eckstreifen, Zwei Nadeln)
  const FASTENER_RULE = { tape: 'quer', tape2: 'fest', thumbtack: 'frei', pushpin: 'frei', pin: 'frei', pin2: 'fest', clip: 'oben', magnet: 'frei', photo: 'frei' };
  // Wie viele von einer Art sinnvoll sind
  const FASTENER_MAX = { tape: 2, tape2: 1, thumbtack: 4, pushpin: 4, pin: 4, pin2: 1, clip: 4, magnet: 4, photo: 4 };
  // Wo eine Befestigung ohne Versatz sitzt – dieselben Werte wie in fastenerShapes
  function fastenerHome(kind, W, H, p) {
    switch (kind) {
      case 'thumbtack': return { x: W / 2, y: -p * 0.12 };
      case 'pushpin':   return { x: W / 2, y: -p * 0.75 };
      case 'pin':       return { x: W / 2 - p * 0.35, y: -p * 0.35 };
      case 'clip':      return { x: W * 0.86, y: p * 0.15 };
      case 'magnet':
      case 'photo':     return { x: W / 2, y: p * 0.3 };
      case 'tape2':
      case 'pin2':      return { x: W / 2, y: 0 };
      default:          return { x: W / 2, y: 0 };   // tape
    }
  }
  const klemm = (v, a, b) => Math.max(a, Math.min(b, v));
  // Eine Wunschposition auf die Regel der Art zwingen
  function fastenerPlace(kind, x, y, W, H, p) {
    const home = fastenerHome(kind, W, H, p);
    switch (FASTENER_RULE[kind]) {
      case 'frei': return { x: klemm(x, 0, W), y: klemm(y, -p * 0.9, H) };
      case 'quer': return { x: klemm(x, 0, W), y: y < H / 2 ? 0 : H };
      case 'oben': return { x: klemm(x, W * 0.12, W * 0.88), y: home.y };
      default:     return home;
    }
  }
  // Wie weit die Befestigungen oben ins Papier ragen (Vielfaches von p): nur was an der Oberkante
  // sitzt, drückt den Text nach unten. Ein Magnet in der Blattmitte tut das nicht – der Text läuft
  // unter ihm durch. Diese Zahl geht in die Schriftgröße ein, also muss sie in App und Server
  // aus derselben Funktion kommen.
  function fastenerInset(liste, W, H, p) {
    let max = 0;
    for (const b of liste || []) {
      const home = fastenerHome(b.art, W, H, p);
      const y = typeof b.y === 'number' ? b.y : home.y;
      if (y <= p) max = Math.max(max, FASTENER_INSET[b.art] || 0);
    }
    return max;
  }
  // Alle Befestigungen einer Liste zeichnen. Jeder Eintrag: { art, x, y, vier?, color? }
  function fastenersShapes(liste, W, H, p, grundLook) {
    return (liste || []).map((b, i) => {
      const home = fastenerHome(b.art, W, H, p);
      const x = typeof b.x === 'number' ? b.x : home.x, y = typeof b.y === 'number' ? b.y : home.y;
      // Farbe und Muster/Motiv dürfen je Stück abweichen – eine rote Klammer neben einer gestreiften
      const look = { ...(grundLook || {}), ...(b.color ? { color: b.color } : {}), ...(b.decor ? { decor: b.decor } : {}) };
      const inhalt = fastenerShapes(b.art, W, H, p, look);
      // Vier Eckstreifen: dieselbe Zeichnung an der Waagerechten gespiegelt, dann stimmen auch die Winkel
      const unten = b.art === 'tape2' && b.vier ? '<g transform="translate(0 ' + H + ') scale(1 -1)">' + inhalt + '</g>' : '';
      const dx = x - home.x, dy = y - home.y;
      // Immer eine eigene Gruppe mit Index: die App misst daran die tatsächliche Ausdehnung jedes
      // Stücks (getBBox) und legt den Griff genau darauf – ein geschätztes Rechteck saß schief.
      const t = dx || dy ? ' transform="translate(' + dx + ' ' + dy + ')"' : '';
      return '<g data-b="' + i + '"' + t + '>' + inhalt + unten + '</g>';
    }).join('');
  }
  function fastenersSvg(liste, W, H, p, grundLook) {
    const m = p * 2.2;
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="' + (-m) + ' ' + (-m) + ' ' + (W + 2 * m) + ' ' + (H + 2 * m) + '" width="' + (W + 2 * m) + '" height="' + (H + 2 * m) + '">' + fastenersShapes(liste, W, H, p, grundLook) + '</svg>';
  }
  ...
  function fastenerShapes(kind, W, H, p, look) {
    ...
    switch (kind) {
      ...
      case 'photo':     return photoShape(W, H, p, L.decor);
  ...
  function fastenerIcon(kind, look) {
    ...
    switch (kind) {
      ...
      case 'photo':     s = PHOTOS[L.decor] ? `<image href="${PHOTOS[L.decor].src}" x="10" y="0" width="40" height="40" preserveAspectRatio="xMidYMid meet"/>` : ''; break;
  ...
  // Für den Server: Lage der Magnetscheibe in Zettel-Koordinaten und der Randglanz als eigenes Fragment.
  // (resvg lädt keine Bilder innerhalb eines eingebetteten SVG; der Server legt das Motiv deshalb als eigenes Bild darüber.)
  function magnetGeometry(W, H, p) { const s = p * 2.1, ox = W / 2, oy = p * 0.3; return { s, ox, oy, cx: ox, cy: oy - 0.04 * s, r: 0.62 * s }; }
  function magnetGlint(W, H, p) {
    const g = magnetGeometry(W, H, p);
    return `<g transform="translate(${g.ox} ${g.oy}) scale(${g.s})"><path d="M-0.5,-0.3 A0.56,0.56 0 0 1 -0.12,-0.58" fill="none" stroke="#ffffff" stroke-opacity=".8" stroke-width="0.05" stroke-linecap="round"/></g>`;
  }
  const motifSrc = key => (MOTIFS[key] ? MOTIFS[key].src : null);
  const photoInfo = key => PHOTOS[key] || null;

  return { FASTENERS, FASTENER_INSET, FASTENER_RULE, FASTENER_MAX, FCOLORS, FCOLOR_DEFAULT, PATTERNS, PATTERN_SUPPORT, DESIGNS, DESIGN_SUPPORT, DECOR_LABEL, decorsFor,
    normalizeFastener, normalizeLook, lookKey, fastenerShapes, fastenerIcon, fastenerSvg, setMotifs, magnetGeometry, magnetGlint, motifSrc, photoInfo, hasColor,
    fastenerHome, fastenerPlace, fastenerInset, fastenersShapes, fastenersSvg };
});
