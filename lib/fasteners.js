...
(function (root, factory) {
  ...
  // Aussehen einer Befestigung: { color, pattern, design }, unpassende Werte fallen auf Standard zurück
  function normalizeLook(kind, look) {
    const l = look || {};
    const c = String(l.color || '').toLowerCase(), p = String(l.pattern || '').toLowerCase(), d = String(l.design || '').toLowerCase();
    const color = FCOLORS[c] ? c : (FCOLOR_ALIAS[c] || FCOLOR_DEFAULT[kind] || 'red');
    let pattern = PATTERNS[p] ? p : (PATTERN_ALIAS[p] || 'none');
    if (!(PATTERN_SUPPORT[kind] || []).includes(pattern)) pattern = 'none';
    let design = DESIGNS[d] ? d : (DESIGN_ALIAS[d] || 'none');
    if (!(DESIGN_SUPPORT[kind] || []).includes(design)) design = 'none';
    return { color, pattern, design };
  }
  const lookKey = look => `${look.color}/${look.pattern}/${look.design}`;
  ...
  const SHAPE = {
    ...
    // Magnet: Scheibe; ohne Motiv ein Glanzfleck, mit Motiv nur ein schmaler Glanz am Rand (nichts liegt über dem Gesicht)
    magnet: (cx, cy, s, c, pat, des) => {
      ...
      return `<g transform="translate(${cx} ${cy}) scale(${s})">${P.defs}
        ...
        ${D ? `<path d="M-0.5,-0.3 A0.56,0.56 0 0 1 -0.12,-0.58" fill="none" stroke="${c.hi}" stroke-opacity=".8" stroke-width="0.05" stroke-linecap="round"/>`
            : `<ellipse cx="-0.2" cy="-0.28" rx="0.22" ry="0.12" fill="${c.hi}" fill-opacity=".8"/>`}</g>`;
    },
  ...
  return { FASTENERS, FASTENER_INSET, FCOLORS, FCOLOR_DEFAULT, PATTERNS, PATTERN_SUPPORT, DESIGNS, DESIGN_SUPPORT,
