...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerColor, fastenerPattern, fastenerDesign, paper = 'plain', pen = 'black', texture = 'smooth', edge = 'straight', fontScale = 100, list = 'none', title = false, titleSize = 1, bg = 'dark', w = 2360, h = 2360 }) {
  ...
  // Linienraster wie fitNote in index.html: auf liniertem/kariertem Papier belegt jede Überschriftzeile eine ganze
  // Zahl Linienzeilen (1 oder 2) ohne Abstand darunter; glattes Papier wie bisher
  const raster = titleOn && paperKind !== 'plain' ? Math.ceil(tF - 1e-9) : 0;
  // Titelzeilen zählen mit dem größeren Faktor (bzw. dem Raster), dazu der Abstand unter der Überschrift
  ...
  if (raster) {
    const zh = fs * F.lh, b = (F.lh + F.asc - F.desc) / 2 * fs, k = Math.max(1, z.t);
    const padTop = (raster - 1) * zh - (tF - 1) * b;
