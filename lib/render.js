...
// Echte Zeichenbreiten der Schrift (Vorschubbreite/em, aus der TTF exportiert), damit die Schriftgrößen-Wahl
// dieselben Umbrüche vorhersagt, die satori dann setzt. Unbekannte Zeichen: Mittelwert.
let widthTable = null;
function charWidths() {
  if (!widthTable) {
    try { widthTable = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'lib', 'caveat-widths.json'), 'utf8')); }
    catch (_) { widthTable = {}; }
  }
  return widthTable;
}
function textWidth(s, fsPx) {
  const t = charWidths(); let w = 0;
  for (const ch of s) w += (t[ch] ?? AVG_GLYPH_EM) * fsPx;
  return w;
}
...
function estimateLines(text, fs, maxW) {
  const space = textWidth(' ', fs);
  let n = 0;
  for (const para of text.split('\n')) {
    if (!para.trim()) { n += 1; continue; }
    let line = 0;
    for (const word of para.split(/\s+/)) {
      const wl = textWidth(word, fs);
      if (line === 0) line = wl;
...
export async function renderZettel({ text, color = 'yellow', w = 2360, h = 2360 }) {
  ...
  // Wie in index.html: 1) kein Wort hart trennen, 2) Absätze möglichst einzeilig (bis noteW/16), 3) Höhe einhalten.
  const paras = text.split('\n');
  const longestWord = paras.flatMap(p => p.split(/\s+/)).reduce((a, b) => (textWidth(b, 100) > textWidth(a, 100) ? b : a), '');
  const minFs = noteW / 28, paraFloor = noteW / 16;
  let fs = Math.round(noteW / 7), lines;
  while (textWidth(longestWord, fs) > textW && fs > minFs) fs = Math.round(fs * 0.94);
  lines = estimateLines(text, fs, textW);
  while (lines > paras.length && fs > paraFloor) { fs = Math.round(fs * 0.95); lines = estimateLines(text, fs, textW); }
  while (lines * fs * LINE_HEIGHT + 2 * pad > layout.maxNoteH && fs > minFs) { fs = Math.round(fs * 0.92); lines = estimateLines(text, fs, textW); }
