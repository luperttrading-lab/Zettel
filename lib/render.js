...
export const FASTENERS = ['tape', 'tape2', 'thumbtack', 'pushpin', 'pin', 'pin2', 'clip', 'magnet'];
const FASTENER_ALIAS = { klebestreifen: 'tape', streifen: 'tape', zwei: 'tape2', ecken: 'tape2', reisszwecke: 'thumbtack', reißzwecke: 'thumbtack', pinnadel: 'pushpin', nadel: 'pin', nadeln: 'pin2', klammer: 'clip', bueroklammer: 'clip', magnet: 'magnet' };
...
function estimateLines(table, text, fs, maxW) {
  ...
  for (const para of text.split('\n')) {
    ...
    const indent = indentOf(table, para, fs);
    let line = 0, first = true;
    const limit = () => (first ? maxW : maxW - indent);
    for (const c of chunksOf(para)) {
      const wl = textWidth(table, c.s, fs), gap = c.sep ? space : 0;
      if (line === 0) line = wl;
      else if (line + gap + wl <= limit()) line += gap + wl;
      else { n += 1; first = false; line = wl; }
      while (line > limit()) { n += 1; first = false; line -= limit(); }
    }
    n += 1;
  }
  return n;
...
// Farben der Befestigungen (siehe index.html): main = Grundton, mid = heller, hi = Glanz, dark/darker = Schatten
export const FCOLORS = {
  red:    { main: '#ff3b30', mid: '#ff5147', hi: '#ffa39d', dark: '#c8281f', darker: '#9c1e17', tape: '#ff3b30' },
  blue:   { main: '#2f7cf6', mid: '#4c90ff', hi: '#9cc4ff', dark: '#1f5fc4', darker: '#174a99', tape: '#2f7cf6' },
  green:  { main: '#34c759', mid: '#4fd370', hi: '#a8f0b8', dark: '#248f40', darker: '#1a6e31', tape: '#34c759' },
  yellow: { main: '#ffcc00', mid: '#ffd633', hi: '#fff0a0', dark: '#cc9f00', darker: '#9a7800', tape: '#ffcc00' },
  white:  { main: '#d8d8de', mid: '#e8e8ec', hi: '#ffffff', dark: '#9a9aa2', darker: '#7a7a82', tape: '#ffffff' },
  black:  { main: '#3a3a40', mid: '#4a4a52', hi: '#8a8a94', dark: '#232327', darker: '#141416', tape: '#3a3a40' },
};
...
export function normalizeFastenerColor(c, kind) {
  const k = String(c || '').toLowerCase();
  return FCOLORS[k] ? k : (FCOLOR_ALIAS[k] || FCOLOR_DEFAULT[kind] || 'red');
}
const partnerColor = key => FCOLORS[key === 'red' ? 'blue' : 'red'];
function fastenerShapes(kind, W, H, p, colorKey) {
  const c = FCOLORS[colorKey] || FCOLORS[FCOLOR_DEFAULT[kind]] || FCOLORS.red;
  const tape = (cx, cy, w, h, deg, c) => `<g transform="rotate(${deg} ${cx} ${cy})"><rect x="${cx - w / 2}" y="${cy - h / 2}" width="${w}" height="${h}" fill="${c.tape}" fill-opacity=".55"/><rect x="${cx - w / 2}" y="${cy - h / 2}" width="${w}" height="${h * 0.12}" fill="#fff" fill-opacity=".25"/></g>`;
  const thumbtack = (cx, cy, s, deg, c) => `<g transform="translate(${cx} ${cy}) rotate(${deg}) scale(${s})">
    ...
    <ellipse cx="0" cy="-0.04" rx="0.6" ry="0.32" fill="${c.mid}"/><ellipse cx="-0.22" cy="-0.16" rx="0.24" ry="0.12" fill="${c.hi}" fill-opacity=".75"/></g>`;
  const pin = (cx, cy, s, deg, c) => `<g transform="translate(${cx} ${cy}) rotate(${deg}) scale(${s})">
    <rect x="-0.05" y="0.3" width="0.1" height="1.5" rx="0.05" fill="#d8d8de"/><rect x="-0.05" y="0.3" width="0.04" height="1.5" fill="#9a9aa2"/>
    <circle cx="0" cy="0" r="0.5" fill="${c.main}"/><circle cx="-0.16" cy="-0.18" r="0.16" fill="${c.hi}" fill-opacity=".8"/>
    <ellipse cx="0.1" cy="0.32" rx="0.34" ry="0.14" fill="#000" fill-opacity=".18"/></g>`;
  const clip = (cx, cy, s, deg, c) => `<g transform="translate(${cx} ${cy}) rotate(${deg}) scale(${s})" fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M-0.3,1.5 V-0.9 a0.3,0.3 0 0 1 0.6,0 V1.6 a0.45,0.45 0 0 1 -0.9,0 V-0.4" stroke="${c.darker}" stroke-width="0.13"/>
    <path d="M-0.3,1.5 V-0.9 a0.3,0.3 0 0 1 0.6,0 V1.6 a0.45,0.45 0 0 1 -0.9,0 V-0.4" stroke="${c.main}" stroke-width="0.08"/></g>`;
  const magnet = (cx, cy, s, c) => `<g transform="translate(${cx} ${cy}) scale(${s})">
    <circle cx="0" cy="0.08" r="0.72" fill="#000" fill-opacity=".25"/><circle cx="0" cy="0" r="0.7" fill="${c.dark}"/>
    <circle cx="0" cy="-0.04" r="0.62" fill="${c.main}"/><circle cx="0" cy="-0.04" r="0.3" fill="${c.dark}"/><ellipse cx="-0.2" cy="-0.28" rx="0.22" ry="0.12" fill="${c.hi}" fill-opacity=".8"/></g>`;
  const pushpin = (cx, cy, s, deg, c) => `<g transform="translate(${cx} ${cy}) rotate(${deg}) scale(${s})">
  ...
  switch (kind) {
    case 'tape2':     return tape(p * 0.7, p * 0.7, W * 0.24, p * 0.8, -45, c) + tape(W - p * 0.7, p * 0.7, W * 0.24, p * 0.8, 45, c); // über die Ecken, etwas ins Papier
    case 'thumbtack': return thumbtack(W / 2, -p * 0.12, p * 1.05, -10, c); // Kopf ragt über die Oberkante
    case 'pushpin':   return pushpin(W / 2, -p * 0.75, p * 1.7, -12, c);
    case 'pin':       return pin(W / 2 - p * 0.35, -p * 0.35, p * 1.25, -28, c);
    case 'pin2':      return pin(W * 0.14, -p * 0.25, p * 1.1, -35, c) + pin(W * 0.86, -p * 0.25, p * 1.1, 35, partnerColor(colorKey));
    case 'clip':      return clip(W * 0.86, p * 0.15, p * 0.95, -8, c);
    case 'magnet':    return magnet(W / 2, p * 0.5, p * 1.05, c);
    default:          return tape(W / 2, 0, W * 0.26, p * 0.9, 1, c);
  }
}
...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerColor, fontScale = 100, list = 'none', w = 2360, h = 2360 }) {
  ...
  const fcolor = normalizeFastenerColor(fastenerColor, kind);
  ...
  while (textWidth(widths, longestWord, fs) > textW && fs > minFs) fs = Math.round(fs * 0.94);
  lines = estimateLines(widths, text, fs, textW);
  const FASTENER_INSET = { tape: 0, tape2: 0, thumbtack: 0.6, pushpin: 0.9, pin: 0.7, pin2: 0.7, clip: 0, magnet: 0.9 };
  ...
  const fastenerSvg = { type: 'svg', props: {
    ...
    children: parseSvgFragment(fastenerShapes(kind, noteW, noteH, pad, fcolor)),
  } };
      ...
      children: [
        ...
        { type: 'div', props: { style: {
          ...
          { type: 'div', props: { style: {
            ...
            fontFamily: 'Note', fontSize: fs, lineHeight: F.lh, color: c.ink,
          }, children: text.split('\n').map(para => {
