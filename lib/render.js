...
function estimateLines(table, text, fs, maxW, titleFs = 0) {
  let n = 0, t = 0;
  text.split('\n').forEach((para, pi) => {
    ...
    const indent = indentOf(table, para, size), limit = maxW - indent;
...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerColor, fastenerPattern, fastenerDesign, paper = 'plain', pen = 'black', texture = 'smooth', edge = 'straight', fontScale = 100, list = 'none', title = false, titleSize = 1, bg = 'dark', w = 2360, h = 2360 }) {
  ...
  const zeilen = f => estimateLines(widths, text, f, textW, titleOn ? Math.round(f * tF) : 0);
  ...
  // Titelzeilen zählen mit dem größeren Faktor, dazu der Abstand unter der Überschrift
  const hoehe = () => ((lines - z.t) + z.t * tF) * fs * F.lh + (titleOn ? fs * tF * TITLE_GAP : 0);
  while (hoehe() + 2 * pad + inset > noteH && fs > minFs) { fs = Math.round(fs * 0.92); z = zeilen(fs); lines = z.n; }
  // overflow wie fitNote in index.html: auch in kleinster Schrift passt der Text nicht – das Bild schneidet ab
  const overflow = hoehe() + 2 * pad + inset > noteH;
      ...
      children: [
        ...
        { type: 'div', props: { style: {
          ...
          { type: 'div', props: { style: {
            ...
            const head = titleOn && pi === 0 ? { textDecoration: 'underline', fontSize: Math.round(fs * tF), marginBottom: Math.round(fs * tF * TITLE_GAP) } : {};
            if (!m) return { type: 'div', props: { style: { ...body, ...head, width: textW, minHeight: fs * F.lh * (titleOn && pi === 0 ? tF : 1) }, children: para || ' ' } };
            const indent = Math.ceil(indentOf(widths, para, titleOn && pi === 0 ? Math.round(fs * tF) : fs));
  ...
  return { png: Buffer.from(png), svg, w, h, layout: layoutName, fontSize: fs, lines, font: fontKey, fastener: kind, title: titleOn, titleSize: tSize, bg: durchsichtig ? 'transparent' : 'dark', overflow };
}
