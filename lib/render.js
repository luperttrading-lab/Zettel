...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fontScale = 100, list = 'none', w = 2360, h = 2360 }) {
  ...
  while (lines * fs * F.lh + 2 * pad + inset > layout.maxNoteH && fs > minFs) { fs = Math.round(fs * 0.92); lines = estimateLines(widths, text, fs, textW); }
  const noteH = Math.max(Math.round(lines * fs * F.lh + 2 * pad + inset), Math.round(noteW * 0.6));
