...
// Flachster Zettel bei textabhängiger Höhe und Grenzen der Neigung – gleiche Werte in index.html
const NOTE_MIN_H = 0.45, NOTE_ROT_STD = -2.5, NOTE_ROT_MAX = 15;
const LAYOUTS = {
...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerPos, fastenerVier, fastenerColor, fastenerPattern, fastenerDesign, paper = 'plain', pen = 'black', texture = 'smooth', edge = 'straight', fontScale = 100, list = 'none', title = false, titleSize = 1, noteScale, noteX, noteY, noteFit, noteRot, bg = 'dark', w = 2360, h = 2360 }) {
  ...
  // Neigung in Grad, gleiche Grenzen wie in index.html
  const nRot = Number.isFinite(+noteRot) ? klemm(+noteRot, -NOTE_ROT_MAX, NOTE_ROT_MAX) : NOTE_ROT_STD;
  // Zwei Regler, zwei Größen (3.41): nScale bestimmt die Breite des Papiers, scale allein die Schrift
  // darauf (weiter unten). Gleiche Rechnung wie fitNote in index.html, wo die Begründung steht.
  ...
  // Schriftgröße an der **Layoutbreite**, nicht an der Zettelbreite (3.42) – gleiche Rechnung wie
  // fitNote in index.html, wo die Begründung steht.
  const bezugW = Math.round(layout.noteW);
  const minFs = bezugW / 28, paraFloor = bezugW / 16;
  let fs = Math.round(bezugW / 7), lines;
  ...
  // Quadratisch wie in index.html – aber nur als **Obergrenze**: steht wenig auf dem Zettel, wird er
  // kürzer statt halb leer (nfit). Ohne den Parameter bleibt es beim alten, quadratischen Verhalten,
  // damit ältere Aufrufe unverändert aussehen.
  const nFit = noteFit === true || noteFit === 1 || noteFit === '1' || noteFit === 'true';
  const noteHMax = Math.round(Math.min(bezugW, layout.maxNoteH));
  ...
  const befListe = (String(fastenerPos || '').split(';').map(t => t.trim()).filter(Boolean).map(t => {
    ...
    // noteHMax, nicht noteH: die endgültige Höhe steht erst nach der Schriftsuche fest (zeitliche
    // Totzone), und die Befestigungen werden ohnehin an der Breite ausgerichtet.
    const q = Fx.fastenerPlace(kind, x / 100 * noteW, y / 100 * noteW, noteW, noteHMax, pad);
  ...
  let inset = Math.round(pad * Fx.fastenerInset(befListe, noteW, noteHMax, pad));
  ...
  // Nicht aufrunden – das gehört allein in die Höhenbestimmung unten, sonst wählt der Server eine andere
  // Schriftgröße als die App (siehe Kommentar in fitNote).
  const brauchtH = () => hoehe() + 2 * pad + inset;
  const passt = f => brauchtH() <= noteHMax && ueberOf(f) <= pad + inset;   // auch: Überschrift bleibt im Papier
  ...
  // Höhe an den Text, gleiche Rechnung wie fitNote in index.html (NOTE_MIN_H = 0,45).
  // Der zweite Durchgang fängt eine Befestigung, die durch die neue Höhe nicht mehr oben sitzt.
  let noteH = noteHMax;
  if (nFit) {
    const hFit = () => Math.max(Math.round(noteW * NOTE_MIN_H), Math.min(noteHMax, Math.ceil(brauchtH())));
    noteH = hFit();
    const i2 = Math.round(pad * Fx.fastenerInset(befListe, noteW, noteH, pad));
    if (i2 !== inset) { inset = i2; noteH = hFit(); }
  }
  // Nur die **Mitte** liegt im Bild (nX, nY sind auf 0…1 geklemmt); der Rest darf über den Rand und
  // wird abgeschnitten – dieselbe Regel wie in index.html seit 3.11. Bis dahin klemmte hier der
  // gedrehte Umriss, damit keine Ecke überhing.
  const left = Math.round(w * nX - noteW / 2);
  const top  = Math.round(h * nY - noteH / 2);
  const m = pad * 2.2;
  ...
  const shadowLayers = [1, 2, 3, 4].map(k => ({ type: 'div', props: { style: {
    ...
    background: 'rgba(0,0,0,0.16)', transform: 'rotate(' + nRot + 'deg)',
  ...
  // satori vererbt `transform` **nicht** an absolut positionierte Kinder (nachgemessen: der Klebestreifen
  // blieb waagerecht, während der Zettel schon um 12° stand). Jede Ebene über dem Papier dreht sich deshalb
  // selbst. Konzentrische SVGs (gleiche Mitte wie der Zettel) brauchen nur die Drehung; kleine Kästchen
  // zusätzlich ihren um die Zettelmitte gedrehten Platz. Im Canvas der App stellt sich die Frage nicht,
  // dort dreht ein einziges ctx.rotate den ganzen Zettel samt Befestigung.
  const gedreht = nRot ? { transform: 'rotate(' + nRot + 'deg)' } : {};
  const drehPlatz = (x, y, bw, bh) => {
    if (!nRot) return { left: Math.round(x), top: Math.round(y) };
    const a = nRot * Math.PI / 180, co = Math.cos(a), si = Math.sin(a);
    const cx = noteW / 2, cy = noteH / 2, mx = x + bw / 2 - cx, my = y + bh / 2 - cy;
    return { left: Math.round(cx + mx * co - my * si - bw / 2), top: Math.round(cy + mx * si + my * co - bh / 2) };
  };
  ...
  if (photo) {
    ...
    for (const v of versatz) {
      motifNodes.push({ type: 'div', props: { style: { position: 'absolute', ...drehPlatz(x + v.dx + size * 0.03, y + v.dy + size * 0.07, size * 0.94, size * 0.94), width: size * 0.94, height: size * 0.94, borderRadius: size * 0.14, background: 'rgba(0,0,0,0.28)', ...gedreht } } });
      motifNodes.push({ type: 'img', props: { src: pngData, width: size, height: size, style: { position: 'absolute', ...drehPlatz(x + v.dx, y + v.dy, size, size), width: size, height: size, objectFit: 'contain', ...gedreht } } });
    }
  }
  if (motif) {
    const g = Fx.magnetGeometry(noteW, noteH, pad), d = Math.round(g.r * 2);
    for (const v of versatz) {
      motifNodes.push({ type: 'div', props: { style: { position: 'absolute', ...drehPlatz(Math.round(g.cx - g.r) + v.dx, Math.round(g.cy - g.r) + v.dy, d, d), width: d, height: d, borderRadius: '50%', overflow: 'hidden', display: 'flex', ...gedreht },
      ...
      motifNodes.push({ type: 'svg', props: { xmlns: 'http://www.w3.org/2000/svg', viewBox: `${-m} ${-m} ${noteW + 2 * m} ${noteH + 2 * m}`, width: noteW + 2 * m, height: noteH + 2 * m,
        style: { position: 'absolute', ...drehPlatz(-m + v.dx, -m + v.dy, noteW + 2 * m, noteH + 2 * m), ...gedreht }, children: parseSvgFragment(Fx.magnetGlint(noteW, noteH, pad)) } });
    }
  }
  const fastenerSvg = { type: 'svg', props: {
    ...
    style: { position: 'absolute', left: -m, top: -m, ...gedreht },   // konzentrisch zum Zettel: Drehung genügt
  ...
  return { png: Buffer.from(png), svg, w, h, layout: layoutName, fontSize: fs, lines, noteW, noteH, rot: nRot, font: fontKey, fastener: kind, title: titleOn, titleSize: tSize, bg: durchsichtig ? 'transparent' : 'dark', overflow, raster };
}
