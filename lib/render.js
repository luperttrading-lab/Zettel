...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerPos, fastenerVier, fastenerColor, fastenerPattern, fastenerDesign, paper = 'plain', pen = 'black', texture = 'smooth', edge = 'straight', fontScale = 100, list = 'none', title = false, titleSize = 1, noteScale, noteX, noteY, noteFit, noteRot, bg = 'dark', w = 2360, h = 2360 }) {
  ...
  // Auf den **gedrehten** Umriss klemmen (|w·cos| + |h·sin|), gleiche Rechnung wie drehMasse in
  // index.html – sonst schneidet das Bild bei schräger Lage am Rand eine Ecke ab.
  const rad = Math.abs(nRot * Math.PI / 180), rc = Math.abs(Math.cos(rad)), rs = Math.abs(Math.sin(rad));
  const dmW = noteW * rc + noteH * rs, dmH = noteW * rs + noteH * rc;
  const left = Math.round(klemm(w * nX, dmW / 2, w - dmW / 2) - noteW / 2);
  const top  = Math.round(klemm(h * nY, dmH / 2, h - dmH / 2) - noteH / 2);
  const m = pad * 2.2;
