...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fastenerColor, fastenerPattern, fastenerDesign, paper = 'plain', pen = 'black', texture = 'smooth', edge = 'straight', fontScale = 100, list = 'none', title = false, w = 2360, h = 2360 }) {
  ...
  const titleGap = () => (titleOn ? fs * TITLE_GAP : 0);
  while (lines * fs * F.lh + titleGap() + 2 * pad + inset > noteH && fs > minFs) { fs = Math.round(fs * 0.92); lines = estimateLines(widths, text, fs, textW); }
      ...
      children: [
        ...
        { type: 'div', props: { style: {
          ...
          { type: 'div', props: { style: {
            ...
            const head = titleOn && pi === 0 ? { textDecoration: 'underline', marginBottom: Math.round(fs * TITLE_GAP) } : {};
            if (!m) return { type: 'div', props: { style: { ...body, ...head, width: textW, minHeight: fs * F.lh }, children: para || ' ' } };
  ...
  return { png: Buffer.from(png), svg, w, h, layout: layoutName, fontSize: fs, lines, font: fontKey, fastener: kind, title: titleOn };
}
