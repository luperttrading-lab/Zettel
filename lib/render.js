...
function loadFont(key) {
  if (!fontCache[key]) {
    ...
    fontCache[key] = {
      ...
      widths: (() => { try { return JSON.parse(fs.readFileSync(path.join(process.cwd(), 'lib', f.widths), 'utf8')); } catch (_) { return {}; } })(),
    };
  }
  return fontCache[key];
}
function textWidth(table, s, fsPx) { let w = 0; for (const ch of s) w += (table[ch] ?? AVG_GLYPH_EM) * fsPx; return w; }
...
export async function renderZettel({ text, color = 'yellow', font = 'caveat', fastener = 'tape', fontScale = 100, w = 2360, h = 2360 }) {
  ...
  const longestWord = paras.flatMap(p => p.split(/\s+/)).reduce((a, b) => (textWidth(widths, b, 100) > textWidth(widths, a, 100) ? b : a), '');
  ...
  while (textWidth(widths, longestWord, fs) > textW && fs > minFs) fs = Math.round(fs * 0.94);
  lines = estimateLines(widths, text, fs, textW);
  while (lines * fs * F.lh + 2 * pad > layout.maxNoteH && fs > minFs) { fs = Math.round(fs * 0.92); lines = estimateLines(widths, text, fs, textW); }
  const noteH = Math.max(Math.round(lines * fs * F.lh + 2 * pad), Math.round(noteW * 0.6));
    ...
    props: {
      ...
      children: [
        ...
        { type: 'div', props: { style: {
          ...
          background: c.paper, transform: 'rotate(-2.5deg)', padding: pad,
        }, children: [
          { type: 'div', props: { style: {
  ...
  const svg = await satori(tree, { width: w, height: h, fonts: [{ name: 'Note', data: fontData, weight: F.weight, style: 'normal' }] });
